...
public class CajeroElectronico{
	...
	public void consultarSaldoTarjeta(TarjetaDebito tarjeta, String password){

		if(tarjeta.verificarPassword(password)){
			System.out.println(" === SALDO EN TARJETA ===");
			System.out.println("SALDO : "+tarjeta.getSaldo());
			this.registrarLog("SALDO", tarjeta.getNumero(), 0, "OK:200");
			System.out.println(" ===== OK - PROCESO REALIZADO CON EXITO =====");
		}else{
			this.registrarLog("SALDO", tarjeta.getNumero(), 0, "Error:403");
			System.out.println(" ===== ERROR - PASSWORD INCORRECTO =====");
		}
	}

	public void cambiarClave(TarjetaDebito tarjeta,String password, String nuevo_password){

		if (tarjeta.verificarPassword(password)){
			tarjeta.cambiarClave(password, nuevo_password);
			this.registrarLog("PASSWORD", tarjeta.getNumero(), 0, "OK:200");
			System.out.println(" == CAMBIO DE CLAVE COON EXITO == ");
		}else{
			this.registrarLog("PASSWROD", tarjeta.getNumero(),0, "Error:403");
			System.out.println(" == CLAVE NO COINCIDE == ");
		}

	}

	public void imprimirDatosTarjetaActualizado(TarjetaDebito numero){
		System.out.println();
		System.out.println("-----------------------------------------");
		System.out.println("-----------------------------------------");
		System.out.println("-- Numero Tarjeta: "+numero.getNumero());
		System.out.println("-- TITULAR : "+numero.getTitular());
		System.out.println("-- lista_transacciones: ");
		for (int i=0; i<this.lista_transacciones.length; i++) {
			if (this.lista_transacciones[i] != null) {
				System.out.println("     => "+this.lista_transacciones[i]);
